...
function Message({
  ...
  if (role === "user") {
    ...
    const isErrorMessage = typeof content === "string" && (content.startsWith("Error:") || content.includes("Failed to load resource") || content.includes("Server returned status"));
    
    if (isErrorMessage) {
      return (
        <div className="assistant-wrap">
          <div className="chat-message error-message-block">
            <div className="error-icon-wrap">
              <FiAlertTriangle className="error-icon" />
            </div>
            <div className="error-content-wrap">
              <p className="error-text">{String(content)}</p>
              {onRegenerate && (
                <button className="error-retry-btn" onClick={() => onRegenerate(messageIndex)}>
                  <GoSync /> Retry Response
                </button>
              )}
            </div>
          </div>
